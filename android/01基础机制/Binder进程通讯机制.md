...
父进程关闭读端，子进程关闭写端；父进程写数据，子进程读数据，管道使用环形队列实现的，数据从写端流入读端，这样实现了进程间通讯。
...
​	Binder是一种进程间通信方式，基于开源OpenBinder实现，是针对手机设计的通信方式。内部使用mmap机制提高性能。
...
- **ServiceManager进程**
  ...
  将字符形式的Binder名字转化为Client中对该Binder的引用，使得Client能够通过Binder名称获取对应Serview的Binder实体的引用。
...
![](/Users/xin/docs/android/images/process_binder.png)
